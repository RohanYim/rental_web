...
class Keycheck_success(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command()
    @commands.has_role('admin')
    async def keycheck_success(self, ctx, bot, key, key_nickname, member: discord.Member = None, *args):
        guild = ctx.guild
        # bot = "Dashe"
        # time = "2021.06.23"
        title = "Your %s key is ready to be listed!" % bot
...
def setup(bot):
    bot.add_cog(Keycheck_success(bot))