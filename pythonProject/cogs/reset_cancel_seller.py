...
class Reset_cancel_seller(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command()
    async def reset_cancel_seller(self, ctx, bot, key, nickname, member: discord.Member = None, *args):
        guild = ctx.guild
        # bot = "Dashe"
        # time = "2016-04-07 10:25:09"
        title = "%s Reset Request Cancelled!" % bot
        embed = discord.Embed(title=title, url="https://www.tidalmarket.com/",
                              color=0x82111f,
                              description="Reset request for %s was cancelled by your buyer" % bot)
...
def setup(bot):
    bot.add_cog(Reset_cancel_seller(bot))
